 ...
 */
    ...
    import {
                retrieveValueFromUnstructuredObject
     ...
     *                  (if false, default)
     * @returns {ex.uString} is the found value (if a string) or undefined if the key doesn't
     *                  exist, if the value is not a string or if an error occurs
     */
    export async function getStorageAsString (kind: StorageKind,
                                              key: string,
                                              forceAsString: boolean = false): Promise<ex.uString> {
